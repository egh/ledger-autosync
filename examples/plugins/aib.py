...
class AIBConverter(CsvConverter):
    FIELDSET = set(["Posted Account", "Posted Transactions Date", "Description",
        "Debit Amount", "Credit Amount", "Balance", "Transaction Type"])
    ...
    def convert(self, row):
        ...
        if (debit and debit > 0):
            posting_from = Posting(self.name, Amount(debit, "€", reverse=True), metadata=meta)
        ...
        payee = re.sub(r"[A-Za-z]+('[A-Za-z]+)?",lambda word: word.group(0).capitalize(),row["Description"])

        return Transaction(
            date=datetime.datetime.strptime(row["Posted Transactions Date"], "%d/%m/%y"),
            cleared=True,
            date_format="%Y-%m-%d",
            payee=payee,
